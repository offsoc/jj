...
pub fn cmd_git_push(
    ...
    let (num_rebased_descendants, bookmark_updates) =
        sign_commits_before_push(&mut tx, bookmark_updates, commits_to_sign, args.dry_run)?;
...
fn sign_commits_before_push(
    ...
    let sign_behavior = if tx.settings().get_bool("git.sign-on-push").unwrap_or(false) {
        SignBehavior::Own
    } else {
        SignBehavior::Keep
    };
    ...
    tx.repo_mut()
        .transform_descendants(commit_ids.iter().cloned().collect_vec(), |rewriter| {
            ...
            if commit_ids.contains(&old_commit_id) {
                ...
                if !dry_run {
                    let commit = rewriter.rebase()?.write()?;
                    old_to_new_commits_map.insert(old_commit_id, commit.id().clone());
                }
            }
            Ok(())
